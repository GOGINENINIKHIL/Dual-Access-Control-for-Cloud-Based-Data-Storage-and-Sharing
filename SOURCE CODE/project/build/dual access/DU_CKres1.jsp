	...
	try
	{
		...
		if(rs1.next())
		{
			...
			if(rs.next())
			{
				...
				{
					key=" Content Key Not Requested !!";
					application.setAttribute("key",key);
					response.sendRedirect("DU_CKres.jsp");
				}
				else if (key.equalsIgnoreCase("Requested"))
				{
					key=" Content Key Not Permitted !!";
					application.setAttribute("key",key);
					response.sendRedirect("DU_CKres.jsp");
				}
				else if (key.equalsIgnoreCase("Permitted"))
				{
					...
					if(rs2.next())
					{		
					   String contentk = rs2.getString(4);
					application.setAttribute("key",contentk);
					response.sendRedirect("DU_CKres.jsp");
					}
				}
			}
